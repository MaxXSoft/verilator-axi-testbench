...
std::pair<std::string, std::string> endpoint(std::string_view text) {
  ...
  if (dot == 0 || dot == text.npos || dot + 1 == text.size()) {
...
struct HostServices::State {
  ...
  std::vector<std::unique_ptr<std::FILE, FileCloser>> files;
#if defined(__unix__) || defined(__APPLE__)
  struct Terminal {
    int descriptor;
    termios original;
  ...
  std::FILE *open(const std::string &path, bool input) {
    ...
    if (path != "-") {
      file = std::fopen(path.c_str(), input ? "rb" : "wb");
      if (!file)
    ...
    if (input) {
      ...
      if (!guarded && ::isatty(fd) && ::tcgetattr(fd, &original) == 0) {
        ...
        raw.c_cc[VMIN] = 0;
        raw.c_cc[VTIME] = 0;
        terminals.push_back({fd, original});
        if (::tcsetattr(fd, TCSANOW, &raw) != 0) terminals.pop_back();
      }
#endif
    }
    return file;
...
DeviceConfig DeviceRegistry::configure(std::string_view name,
  ...
  for (const auto &option : schema) {
    auto found = properties.find(option.name);
    const auto value =
...
void register_builtin_devices(DeviceRegistry &registry) {
  const auto memory_options = std::vector<DeviceOption>{
      {"size", OptionKind::Unsigned, "65536", "Memory capacity in bytes", 1,
       std::numeric_limits<std::size_t>::max()},
      {"image", OptionKind::String, "",
       "Raw image loaded at the device's sole mapping"}};
  registry.add("rom", {memory_options,
                       [](const DeviceConfig &c, HostServices &) {
                         return std::make_unique<RomDevice>(
                             static_cast<std::size_t>(c.number("size")));
                       },
                       "image"});
  registry.add("ram", {memory_options,
                       [](const DeviceConfig &c, HostServices &) {
...
PlatformSpec default_platform(const DefaultMap &map) {
  PlatformSpec spec;
  spec.devices = {{"rom", "rom", {{"size", std::to_string(map.rom_size)}}},
                  {"ram", "ram", {{"size", std::to_string(map.ram_size)}}},
                  {"uart", "uart", {}},
                  {"exit", "exit", {}}};
  spec.mappings = {{"rom", map.rom_base, map.rom_size},
                   {"ram", map.ram_base, map.ram_size},
                   {"uart", map.uart_base, map.uart_size},
                   {"exit", map.exit_base, map.exit_size}};
  return spec;
}
...
Platform::Platform(const DeviceRegistry &registry, const PlatformSpec &spec,
  ...
  for (const auto &d : spec.devices) {
    validate_name(d.id);
    if (!configs.emplace(d.id, registry.configure(d.type, d.properties)).second)
  ...
  for (const auto &d : spec.devices) {
    ...
    if (!created) throw std::invalid_argument("factory returned null: " + d.id);
    devices_.emplace(d.id, std::move(created));
  }
  for (const auto &m : spec.mappings) {
    if (address_bits == 32 && (m.base >= (UINT64_C(1) << 32) ||
                               m.size > (UINT64_C(1) << 32) - m.base))
  ...
  for (const auto &d : spec.devices) {
    ...
    if (key.empty() || configs.at(d.id).text(key).empty()) continue;
    const auto count =
    ...
    images_.push_back({configs.at(d.id).text(key), m->base});
  }
  std::map<Device *, std::vector<Device *>> successors;
  std::map<Device *, unsigned> indegree;
  for (const auto &d : spec.devices) indegree[&device(d.id)] = 0;
  ...
  for (const auto &d : spec.devices)
    if (indegree[&device(d.id)] == 0) order_.push_back(&device(d.id));
  for (std::size_t i = 0; i < order_.size(); ++i)
    for (auto *next : successors[order_[i]])
      if (--indegree[next] == 0) order_.push_back(next);
