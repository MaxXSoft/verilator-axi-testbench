...
int main() {
  ...
  auto write = [&](std::uint64_t offset, std::uint8_t value) {
  ...
  auto irq = [&] { return uart.output("irq")->value; };
  assert(irq() == 0);
  ...
  assert(irq() == 0);
  for (unsigned i = 0; i < 7; ++i) uart.tick();
  assert(irq() == 0);
  uart.tick();
  ...
  for (unsigned i = 0; i < 4; ++i) backend.push_input('b' + i);
  uart.tick();
  assert(irq() == 1 && read(2) == 0xc4);
  assert(read(0) == 'b' && irq() == 0);
  write(1, 0);
  for (unsigned i = 0; i < 10; ++i) uart.tick();
  assert(irq() == 0);
  write(1, 1);
  assert(irq() == 1);
